...
def update():
    ...
    if pwngef.arch.ptrsize == 4:
        Ehdr = pwngef.elftypes.Elf32_Ehdr
        Phdr = pwngef.elftypes.Elf32_Phdr
    else:
        Ehdr = pwngef.elftypes.Elf64_Ehdr
        Phdr = pwngef.elftypes.Elf64_Phdr
...
def read(typ, address, blob=None):
    ...
    if not blob:
        data = pwngef.memory.read(address, size)
    else:
...
@pwngef.memoize.reset_on_objfile
def get_elf_info(filepath):
    ...
    """
    local_path = pwngef.file.get_file(filepath)
...
def entry():
    ...
    """
    entry = pwngef.auxv.get().AT_ENTRY
    if entry:
        return entry

    # Looking for this line:
    # Entry point: 0x400090
    for line in pwngef.info.files().splitlines():
    ...
    for name in ['_start', 'start', '__start', 'main']:
        try:
            return pwngef.symbol.address(name)
        except gdb.error:
            pass
...
def find_elf_magic(pointer, max_pages=1024, search_down=False, ret_addr_anyway=False):
    ...
    """
    addr = pwngef.memory.page_align(pointer)
    step = pwngef.memory.PAGE_SIZE
    if search_down:
        step = -step
    ...
    for _ in range(max_pages):
        ...
        try:
            data = pwngef.memory.read(addr, 4)
...
def get_ehdr(pointer):
    ...
    # the ELF header.
    base = pwngef.memory.page_align(pointer)
    ...
    ei_class = pwngef.memory.byte(base + 4)
...
def map(pointer, objfile=''):
    ...
    Returns:
        A sorted list of pwngef.memory.Page objects

    Example:

        >>> pwngef.elf.load(pwngef.regs.pc)
        [Page('400000-4ef000 r-xp 0'),
         Page('6ef000-6f0000 r--p ef000'),
         Page('6f0000-6ff000 rw-p f0000')]
        >>> pwngef.elf.load(0x7ffff77a2000)
...
def map_inner(ei_class, ehdr, objfile):
    ...
    for phdr in iter_phdrs(ehdr):
        ...
        vaddr = pwngef.memory.page_align(vaddr)
        offset = pwngef.memory.page_align(offset)

        # For each page described by this program header
        for page_addr in range(vaddr, vaddr + memsz, pwngef.memory.PAGE_SIZE):
            if page_addr in pages:
                ...
                page = pwngef.memory.Page(page_addr, pwngef.memory.PAGE_SIZE, flags, offset + (page_addr - vaddr))
                pages.append(page)
    ...
    for i in range(len(pages) - 1):
        ...
        b_begin = b.vaddr
        if a_end != b_begin:
            gaps.append(pwngef.memory.Page(a_end, b_begin - a_end, 0, b.offset))
